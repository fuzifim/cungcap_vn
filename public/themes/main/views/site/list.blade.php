<?
Theme::setTitle('Danh sách trang web'); 
Theme::setSearch('site'); 
Theme::setType('website'); 
Theme::setCanonical(route('site.list',config('app.url'))); 
$string='<script type="application/ld+json">
		...
		"url" : "'.route('site.list',config('app.url')).'"
		}
		</script>
	'; 
...
<div class="container">
	<div class="form-group">
		<h1 class="text-light">Danh sách trang web</h1> 
	</div>
				...
				@if(count($getNote)>0) 
					...
					@foreach($getNote as $item)
						<li class="list-group-item">
							<h2><a href="{{route('site.show',array(config('app.url'),$item['_id'],str_slug($item['title'], '-')))}}">{!!$item['title']!!}</a></h2>
							<?
