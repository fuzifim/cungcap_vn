<?php
	if(!empty($note->id)){
		...
		$title='Tạo kênh Cung Cấp'; 
		$buttonSave='Tạo ngay'; 
	}
	Theme::setTitle($title); 
	Theme::setSearch('channel:add'); 
				...
				<div class="card-body p-2">
					<form id="postForm" class="" method="POST">
					  ...
					  <div class="form-group">
						<input type="text" class="form-control formTitle" id="inputTitle" name="inputTitle" placeholder="Nhập tên kênh của bạn..." value="@if(!empty($note->id)){{$note->title}}@endif" required>
					  </div>
					  <div class="form-group">
						<textarea id="summernote" class="form-control" name="postContent" placeholder="Nhập nội dung mô tả kênh của bạn..." required>@if(!empty($note->content)){{html_entity_decode($note->content)}}@endif</textarea>
					  </div>
					  <div class="form-group">
	...
	Theme::asset()->writeScript('loadLazy',' 
		...
		$("#savePost").on("click", function () {
			...
			if(!$valid) {
				...
				var formData = new FormData();
				formData.append("channelId", $("input[name=postId]").val()); 
				formData.append("channelTitle", postTitle); 
				formData.append("channelContent", postContent); 
				...
				$.ajax({
					url: "'.route("channel.add.request",config('app.url')).'",
					headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
					type: "post",
					cache: false,
					contentType: false,
