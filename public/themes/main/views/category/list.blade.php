<?
Theme::setTitle('Danh sách danh mục'); 
Theme::setSearch('category'); 
Theme::setType('website'); 
Theme::setCanonical(route('category.list',config('app.url'))); 
$string='<script type="application/ld+json">
		...
		"url" : "'.route('category.list',config('app.url')).'"
		}
		</script>
	'; 
...
<div class="container">
	<div class="form-group">
		<h1 class="text-light">Danh sách danh mục</h1> 
	</div>
				...
				@if(count($getNote)>0) 
					...
					@foreach($getNote as $item)
						<li class="list-group-item">
							<h2><a href="{{route('show.category',array(config('app.url'),str_replace(' ','+',$item['title'])))}}">{!!$item['title']!!}</a></h2>
							<?
