<?
Theme::setTitle('Cung cấp tin tức'); 
Theme::setSearch('news'); 
Theme::setType('website'); 
Theme::setCanonical(route('news.list',config('app.url'))); 
$string='<script type="application/ld+json">
		{
		"@context" : "http://schema.org",
		"@type" : "WebSite",
		"name" : "Cung cấp tin tức",
		"alternateName" : "",
		"dateModified": "'.date('Y-m-d\TH:i:sP', strtotime(\Carbon\Carbon::now())).'",
		"url" : "'.route('news.list',config('app.url')).'"
		}
		</script>
	'; 
...
<div class="container">
	<div class="form-group">
		<h1 class="text-light">Cung cấp tin tức</h1> 
	</div>
				...
				@if(count($getNote)>0) 
					...
					@foreach($getNote as $item)
						<div class="list-group-item">
							<strong><a class="title" href="{{route('news.show',array(config('app.url'),$item['_id'],str_slug($item['title'], '-')))}}">{!!$item['title']!!}</a></strong>
						</div>
					@endforeach 
					</div>
					<div class="form-group">
