<?
Theme::setTitle('Danh sách kênh cung cấp'); 
Theme::setSearch('channel'); 
Theme::setType('website'); 
Theme::setCanonical(route('channel.list',config('app.url'))); 
$string='<script type="application/ld+json">
		{
		"@context" : "http://schema.org",
		"@type" : "WebSite",
		"name" : "Danh sách kênh cung cấp",
		"alternateName" : "",
		"dateModified": "'.date('Y-m-d\TH:i:sP', strtotime(\Carbon\Carbon::now())).'",
		"url" : "'.route('channel.list',config('app.url')).'"
		}
		</script>
	'; 
...
<div class="container">
	<div class="form-group">
		<h1 class="text-light">Danh sách kênh cung cấp</h1> 
	</div>
				...
				@if(count($getNote)>0) 
					...
					@foreach($getNote as $item)
						<div class="list-group-item">
							<strong><a class="title" href="{{route('profile',array(config('app.url'),$item['_id']))}}">{!!$item['title']!!}</a></strong>
						</div>
					@endforeach 
					</div>
					<div class="form-group">
